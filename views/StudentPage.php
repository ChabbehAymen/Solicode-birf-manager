...
<body class="p-2.5 h-screen" style="box-sizing:border-box;">
    <div class="flex  h-full">
        ...
        <div class="w-4/5 ml-60 flex flex-wrap gap-4">
            <?php
                ...
                foreach($Realisers as $Realiser)
                {
                    ...
                    require "views/layout-student/card.php";
                }                     
            ?>
        </div>
        <?php
            } elseif(isset($_POST['CurrentBriefs'])) {
        ?>
            <div class="w-4/5 ml-60 flex flex-wrap gap-4">
            <?php
                $Connexion = new PDO('mysql:host=localhost;dbname=bp15', "root", "A20002024");
                $StatementRealiser = $Connexion->prepare("select * from realiser inner join brief 
                using(id_brief)
                where id_apprenant = :idApprenant and date(DATE_DEBUT+DUREE) > now()");
                $StatementRealiser->execute(
                    [
                        "idApprenant"=>$_SESSION['user']['id']
                    ]
        ...
        <?php
        }elseif(isset($_GET['idBrief'])){  
                $Connexion = new PDO('mysql:host=localhost;dbname=bp15', "root", "A20002024");
                $StatementBrief = $Connexion->prepare("select * from realiser inner join brief 
                using(id_brief) inner join concerne using(id_brief) inner join competence using (id_competence)
                where id_brief = :idbrief and id_apprenant = :idApprenant");
                $StatementBrief->execute(
                    [
                        "idbrief"=>$_GET['idBrief'],
                        "idApprenant"=>$_SESSION['user']['id']
                    ]
                );
                $Brief = $StatementBrief->fetch(PDO::FETCH_OBJ);
                $StatementCompetences = $Connexion->prepare("select * from concerne inner join competence using(id_competence)
                where  id_brief = :idbrief");
                $StatementCompetences->execute(
                    [
                        "idbrief"=>$_GET['idBrief']
                    ]
        ...
        ?>
            ...
            <div class="h-full w-1/2">
                <img src="views/images/ClassImage.png" alt="" class="w-100">
                <br>
            ...
            <div class="h-full w-1/2 p-5">
                ...
                <div class="flex flex-wrap justify-end">
                    ...
                    <?php 
                    $today = new DateTime();
                    $dateString = $Brief->DATE_DEBUT;
                    $dateObject = new DateTime($dateString);
                    if($today <= $dateObject->modify('+'.$Brief->DUREE.' days')){echo "id=\"EtatButton\"";}
                    ?>
                    
                    class="rounded-lg  p-1 pe-3 ps-3  cursor-pointer text-center
                    <?= ($Brief->ETAT=="To do")?"bg-gray-200"
                    :(($Brief->ETAT=="Doing")?"bg-green-200"
                    :(($Brief->ETAT=="Done")?"bg-blue-200":"bg-red-200")) ?>
                    "><?= $Brief->ETAT?></p>
                </div>
                <br>
                <div>
                    <small class="text-blue-500">Start date:</small>
                    <p class='text-2xl'><?php $dateString = $Brief->DATE_DEBUT;
                        $dateObject = new DateTime($dateString);
                        $formattedDate = $dateObject->format('l d F Y');
                        echo $formattedDate?>
                    </p>
                </div>
                <div>
                    ...
                    <p class='text-2xl'><?php
                        $dateObject->modify('+'.$Brief->DUREE.' days');
                        $formattedDate = $dateObject->format('l d F Y');
                        echo $formattedDate?>
                    </p>
                </div>
                ...
                <br>
                <div>
                    <button type="button" id="AddlinkIcon" >
                    ...
                    <form class="flex flex-col gap-5 visible" id="FormAddLink">
                            <div>
                                <label class="block text-gray-700 text-sm font-bold mb-2" for="LienInput">Lien:</label>
                                <input class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="LienInput" type="text" placeholder="Lien" value="<?= $Brief->LIEN?>">
                            </div>
                            <div class="self-end">
                                <button type="button" 
                                <?php 
                                    ...
                                    $dateString = $Brief->DATE_DEBUT;
                                    $dateObject = new DateTime($dateString);
                                    if($today <= $dateObject->modify('+'.$Brief->DUREE.' days')){echo "id=\"ValiderLien\"";}
                                ?>
...
<script src="views/js/EtatBrief.js"></script>
</html>